...
final class OtherEarnings {
	...
	public function __construct() {
		\add_action( 'rest_api_init', [ $this, 'register_api_other_earnings' ] );
	}
	...
	protected function get_apis() {
		return [
			...
			[
				'endpoint'            => 'other_earnings/(?P<id>\d+)',
				'method'              => 'post',
				'permission_callback' => '__return_true', // TODO 應該是特定會員才能看
			],
			[
	...
	/**
	 * Register other earnings API
	 *
	 * @return void
	 */
	public function register_api_other_earnings(): void {
		$this->register_apis(
		apis: $this->get_apis(),
	...
	/**
	 * Get other earnings callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 * @phpstan-ignore-next-line
	 */
	public function get_other_earnings_callback( $request ) { // phpcs:ignore
		$params = $request->get_query_params() ?? [];
		$params = WP::sanitize_text_field_deep( $params, false );
		// 查詢 Custom Post Type 'other_earnings' 的文章
		$args = [
		...
		if (isset($params['meta_query'])) {
			$args['meta_query'] = Base::sanitize_meta_query($params['meta_query']);
		}
		...
		if ($query->have_posts()) {
			while ($query->have_posts()) {
				...
				// 整理 meta 資料
				foreach (PostType\OtherEarnings::instance()->get_meta() as $key => $value) {
	...
	/**
	 * Create other earnings callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function post_other_earnings_callback( $request ) { // phpcs:ignore
		$params = $request->get_json_params() ?? [];
		$params = WP::sanitize_text_field_deep( $params, false );
		// 創建文章
		$post_id = wp_insert_post(
			[
				'post_type'    => 'other_earnings', // 自定義文章類型名稱
				'post_title'   => isset($params['remark'])?$params['remark']:'', // 文章標題
				'post_content' => '', // 文章內容
	...
	/**
	 * Update other earnings callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function post_other_earnings_with_id_callback( $request ) { // phpcs:ignore
		$params     = $request->get_json_params() ?? [];
		$params     = WP::sanitize_text_field_deep( $params, false );
		$post_id    = $request->get_param('id');
		$post_title = isset($params['remark'])?$params['remark']:\get_the_title($post_id);
		// 更新文章
	...
	/**
	 * Get other earnings by id callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function get_other_earnings_with_id_callback( $request ) { // phpcs:ignore
		$post_id = $request->get_param('id');
		$args    = [
			'post_type' => 'other_earnings',  // 指定自定义文章类型
	...
	/**
	 * Delete other earnings by id callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function delete_other_earnings_with_id_callback( $request ) { // phpcs:ignore
		$post_id = $request->get_param('id');
		// 刪除文章
