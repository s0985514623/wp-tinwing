...
final class DebitNotes {
	...
	protected function get_apis() {
		return [
			...
			[
				'endpoint'            => 'debit_notes/not_receipt',
				'method'              => 'get',
				'permission_callback' => '__return_true', // TODO 應該是特定會員才能看
			],
			[
				'endpoint'            => 'debit_notes/and_credit_notes',
				'method'              => 'get',
				'permission_callback' => '__return_true', // TODO 應該是特定會員才能看
			],
		];
	...
	/**
	 * Get debit_notes & Credit_notes not receipt callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function get_debit_notes_not_receipt_callback( $request ) { // phpcs:ignore
		$params = $request->get_query_params() ?? [];
		$params = WP::sanitize_text_field_deep( $params, false );
	...
	/**
	 * Get debit_notes & Credit_notes  callback
	 *
	 * @param \WP_REST_Request $request Request.
	 * @return \WP_REST_Response
	 */
	public function get_debit_notes_and_credit_notes_callback( $request ) { // phpcs:ignore
		$params = $request->get_query_params() ?? [];
		$params = WP::sanitize_text_field_deep( $params, false );
		// error_log(print_r($params, true));
		// 查詢 Custom Post Type 'debit_notes' 和 'credit_notes' 的文章
