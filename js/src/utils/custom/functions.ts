...
export const getGrossPremium = ({
  ...
  return round(
    Number(premium) * (1 + Number(ls) / 100) * (1 + Number(ncb) / 100) + 1e-10,
    2,
  )
}

export const getGrossPremiumByDebitNote = (
  debitNote: TDebitNote | undefined,
) => {
  ...
  return !!debitNote
    ? round(premium * (1 + ls / 100) * (1 + ncb / 100) + 1e-10, 2)
    : 0
}

export const getMotorTotalPremium = ({
  grossPremium,
  mib,
  less,
  extraValue = 0,
  ...
  const preciseAmount =
    Number(grossPremium) * (1 + (Number(mib) + Number(extraValue)) / 100) +
    Number(less)
  // 加上微小偏移後再四捨五入
  const result = round(preciseAmount + 1e-10, 2)
  return result
}

export const getMotorTotalPremiumByDebitNote = (
  debitNote: TDebitNote | undefined,
) => {
  ...
  return !!debitNote
    ? round(grossPremium * (1 + (mib + extraValue) / 100) + less + 1e-10, 2)
    : 0
}

export const getGeneralTotalPremium = ({
  premium,
  levy,
  less,
  extraValue = 0,
  ...
  const preciseAmount =
    Number(premium) *
      (1 + (Number(levy) + Number(extraValue) + Number(extraValue2)) / 100) +
    Number(less)
  // 加上微小偏移後再四捨五入
  const result = round(preciseAmount + 1e-10, 2)
  return result
...
export const getGeneralTotalPremiumByDebitNote = (
  debitNote: TDebitNote | undefined,
) => {
  const premium = Number(debitNote?.premium) || 0
  ...
  const extraValue = Number(debitNote?.extra_field?.value) || 0
  return !!debitNote
    ? round(premium * (1 + (levy + extraValue) / 100) + less + 1e-10, 2)
    : 0
}

export const getTotalPremiumByDebitNote = (
  debitNote: TDebitNote | TRenewal | undefined,
) => {
 ...
 */
export const getInsurerPayment = (
  receipt: TReceipt,
