...
export const ListView: React.FC = () => {
    ...
    //Export CSV
    const { triggerExport: _Export, isLoading: _exportLoading } = useExport<DataType>({
        mapData: (item) => {
            return {
                ...item,
    ...
    //取得資料
    const { data: debitNotesData, isLoading: debitNotesIsLoading } = useList<TDebitNote>({
        resource: 'debit_notes',
        filters: filters as CrudFilters,
    });
    const { data: quotationsData, isLoading: quotationsIsLoading } = useList<TQuotations>({
        resource: 'quotations',
        filters: filters as CrudFilters,
    });
    const { data: receiptsData, isLoading: receiptsIsLoading } = useList<TReceipts>({
        resource: 'receipts',
        filters: dateRange
            ? [
                  {
    ...
    const { data: expensesData, isLoading: expensesIsLoading } = useList<TExpenses>({
        resource: 'expenses',
        filters: dateRange
            ? [
                  {
    ...
    const { data: insurersData, isLoading: insurersIsLoading } = useList<TInsurers>({
        resource: 'insurers',
    });
    ...
    const formatTotalIncome = useFormatLineGridData({ data: receiptsData?.data as TReceipts[], type: 'totalIncome', debitNotesData: debitNotesData?.data });

    //5.Insurer Payment [要給保險的錢](insurerTotalFee)
    const formatInsurerPayment = useFormatLineGridData({ data: receiptsData?.data as TReceipts[], type: 'insurerPayment', debitNotesData: debitNotesData?.data, insurerData: insurersData?.data });

    //6.Expense 在Expense輸入了各項Expense的總和[費用紀錄金額]
    const formatTotalExpense = useFormatLineGridData({ data: expensesData?.data as TExpenses[], type: 'totalExpense' });

    //7.Net Income [收入] 扣掉 [要給保險的錢] 扣掉 [費用紀錄金額](就是依照時間篩出來的第6項總和)
    const totalIncome = formatTotalIncome.reduce((acc, item) => acc + item.value, 0);
    const totalExpense = formatTotalExpense.reduce((acc, item) => acc + item.value, 0);
    const totalInsurerPayment = formatInsurerPayment.reduce((acc, item) => acc + item.value, 0);
    ...
    const incomeByBankReceipt = [
        ...
        ...(receiptsData?.data.reduce((acc, receipt) => {
            //取得銀行名稱
            const bank = receipt?.payment_receiver_account;
            //有銀行名稱才執行
            if (!bank) return acc;
            //取得premium
            const income = receipt?.premium ? Number(receipt?.premium) : getTotalPremiumByDebitNote((debitNotesData?.data?.filter((debitNote) => debitNote?.id === receipt?.debitNoteId) as TDebitNote[])[0] ?? 0);
            // 尋找是否已經有該銀行的紀錄
    ...
    //結合所有數據並排序
    const allData = _.sortBy([...formatTotalIncome, ...formatTotalExpense, ...formatInsurerPayment], ['date', 'value']);

    //是否顯示Spin
    const isLoading = debitNotesIsLoading || quotationsIsLoading || receiptsIsLoading || expensesIsLoading || insurersIsLoading;
    //顯示數據
    const ShowData = () => {
        if (allData.length === 0) {
            ...
            <>
                <div className="grid grid-cols-3 gap-5 my-4">
                    <div className="flex flex-col w-full shadow-md bg-white rounded-lg p-5">
                        <div className="text-2xl text-slate-700 font-bold">{netIncome.toLocaleString()}</div>
